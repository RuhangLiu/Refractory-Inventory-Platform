...
- Managed tools: private BigQuery-backed `refractory-inventory-mcp`, native
  `gcs-remote-mcp`, and Vertex AI RAG retrieval
...
2. Use `get_inventory_snapshot` from the private Inventory MCP service to retrieve structured inventory facts from `refractory-inventory-platform.kaixiang_inventory.serving_inventory`. Restrict the request to the requested product code and warehouse. The tool is deterministic and read-only and may not change data.
