...
CREATE OR REPLACE MODEL `YOUR_PROJECT_ID.kaixiang_inventory.steel_demand_forecast`
OPTIONS (
...
FROM `YOUR_PROJECT_ID.kaixiang_inventory.steel_industry_index`
WHERE steel_production_index IS NOT NULL;

CREATE OR REPLACE TABLE `YOUR_PROJECT_ID.kaixiang_inventory.serving_demand_forecast` AS
SELECT
  observation_date,
...
WHERE observation_date >= DATE_SUB(
  (SELECT MAX(observation_date) FROM `YOUR_PROJECT_ID.kaixiang_inventory.steel_industry_index`),
  INTERVAL 59 MONTH
)
UNION ALL
SELECT
...
FROM ML.FORECAST(
  MODEL `YOUR_PROJECT_ID.kaixiang_inventory.steel_demand_forecast`,
  STRUCT(12 AS horizon, 0.90 AS confidence_level)
);

SELECT *
FROM `YOUR_PROJECT_ID.kaixiang_inventory.serving_demand_forecast`
