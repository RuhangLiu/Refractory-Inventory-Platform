...
CREATE OR REPLACE VIEW `YOUR_PROJECT_ID.refractory_inventory.data_quality_results` AS
SELECT
...
FROM (
  SELECT product_code
  FROM `YOUR_PROJECT_ID.refractory_inventory.products`
...
FROM (
  ...
  FROM `YOUR_PROJECT_ID.refractory_inventory.inventory`
...
FROM `YOUR_PROJECT_ID.refractory_inventory.transactions` AS t
LEFT JOIN `YOUR_PROJECT_ID.refractory_inventory.products` AS p
  USING (product_code);

-- Expected result: every failing_row_count equals zero.
SELECT *
FROM `YOUR_PROJECT_ID.refractory_inventory.data_quality_results`
ORDER BY check_name;
